...
4. **截图目录权限**：需确保 `config.SCREENSHOT_DIR` 拥有写入权限。
...
7. **输出标准**：执行完截图后不要有多余的图片分析和内容分析，直接告知是否成功，执行截图后输出示例：
`截图成功，保存路径：C:\data\tools\onmyoji-bot\tmp\screenshots\screenshot_20260531_214353.png`，即可，如果有多条方法需要按顺序执行则可以不输出，等待完整任务完成后对整个任务完成情况进行输出
